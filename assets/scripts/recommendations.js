function giveARecommendation(subject) {
    ...
    if (examSoon && studentGrade < 75) {
        ...
        steps.push('ذاكر يوميًا 45 دقيقة (مراجعة مركزة وليست قراءة عشوائية).');
        steps.push('حل نموذج امتحان أو راجع 20 سؤالًا كل يومين.');
        steps.push('اكتب أخطاءك في ورقة وراجعها 10 دقائق في نهاية اليوم.');
    } 
    else if (veryWeak && isHard) {
        title = 'ابدأ من الأساسيات';
        focus = 'المشكلة غالبًا في الفهم قبل التدريب، فابدأ تدريجيًا.';
        steps.push('30 دقيقة: ركّز على نقطة واحدة فقط (قاعدة/قانون) واكتب ملخصًا قصيرًا.');
        steps.push('بعدها: حل 10 مسائل سهلة جدًا على نفس النقطة.');
        steps.push('تجنب المسائل الصعبة الآن؛ التدرج أهم من الكثرة.');
    } 
    else if (veryWeak && isEasy) {
        title = 'رفع سريع لأن المادة سهلة';
        focus = 'غالبًا تحتاج تدريبًا أكثر وتجنب التسرع.';
        steps.push('حل 20 سؤالًا قصيرًا يوميًا (متنوعًا).');
        steps.push('راجع الأخطاء فورًا، وسجل سبب الخطأ لا الإجابة فقط.');
        ...
        steps.push('25 دقيقة يوميًا ثابتة مهما كانت الظروف.');
        steps.push('كل يوم هدف واحد: درس صغير أو 10 مسائل.');
        steps.push('مرة أسبوعيًا: راجع ورقة الأخطاء/الملخصات 20 دقيقة.');
    } 
    else if (isHard && (isNeglected || isLowTime)) {
        title = 'قسّم المذاكرة لتقليل الضغط';
        focus = 'المواد الصعبة تحتاج جلسات قصيرة مع تطبيق مباشر.';
        steps.push('جلسات 25 دقيقة + 5 دقائق راحة (مرتين يوميًا إن استطعت).');
        ...
        steps.push('حل مسائل متوسطة المستوى (لا سهلة جدًا ولا صعبة جدًا).');
        ...
        steps.push('حل نموذجًا كاملًا واحدًا وراجع الأخطاء فقط.');
        steps.push('اكتب ملخصًا من صفحة واحدة لأهم القوانين/القواعد.');
        ...
        focus = 'هذه مادة قوية لديك، فاجعلها صيانة لا استنزافًا للوقت.';
        steps.push('مراجعة خفيفة مرة أسبوعيًا 30 دقيقة.');
        ...
        focus = 'أنت تبذل وقتًا زائدًا في مادة أنت جيد فيها، ويمكن استثمار الوقت في مادة أضعف.';
        ...
        steps.push('مراجعة مرتين أسبوعيًا 30 إلى 45 دقيقة.');
        steps.push('حل 10 أسئلة بعد كل جزء تراجعه.');
        steps.push('سجّل الأخطاء وارجع لها أسبوعيًا.');
    }
